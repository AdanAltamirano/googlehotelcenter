<%@ Control Language="vb" AutoEventWireup="false" Codebehind="ctrlPlanFaresNR.ascx.vb" Inherits="RateManager.ctrlPlanFaresNR" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
<!--
  ...
  function formatAsMoney(mnt) {
    ...
    function <%= Me.id %>_ValidateRates() {
        ...
        minPercent = Number($('#<%= varMinPercent.ClientId %>').val());
        if (isNaN(minPercent))
            minPercent = 0;
            jj();
        var isValid = true;
        
        $('#<%= Me.dgAdult.ClientId %> tr, #<%= Me.dgChild.ClientId %> tr, #<%= Me.dgTeen.ClientId %> tr').each(function(){
            
            if($(this).find('input.netrate').length > 0){
                
                var netRate = Number($(this).find('input.netrate'). val());
                var rate = Number($(this).find('input.rate:hidden').val());
                 whil()                
                if (isNaN(netRate)) netRate = 0;
                if (isNaN(rate)) rate = 0;
                if (rate <= 0 || (((rate - netRate) * 100) / netRate) < minPercent){
                    isValid = false;               
                    $(this).find('span.message').show();
                    $(this).find('span.message').html('<%= Ratemanager.PortalCulture.GetString("01135") %>');
                } else if(rate <= netRate ) {
                alert(999);
                }
                 else {
		...
		<TD vAlign="top" align="center" ><asp:datagrid id="dgAdult" runat="server" CssClass="DataGrid" AutoGenerateColumns="False" Width="100%">
				...
				<Columns>
					...
					<asp:TemplateColumn HeaderText="Tarifa NR">
						...
						<ItemTemplate>
							<asp:TextBox id="txtAdultFareNR" CssClass="TextBox currency netrate" runat="server" MaxLength="10" Columns="10" Text='<%# DataBinder.Eval(Container, "DataItem.priceNR") %>'>
							</asp:TextBox>
					...
					<asp:TemplateColumn HeaderText="Tarifa UV">
						...
						<ItemTemplate>
							<asp:TextBox id=txtAdultFare CssClass="TextBox currency rate" runat="server" MaxLength="10" Columns="10" Text='<%# DataBinder.Eval(Container, "DataItem.price") %>'>
							</asp:TextBox>
							<span class="currency"></span>
					...
					<asp:TemplateColumn Visible="True" HeaderText="">
						...
						<ItemTemplate>
							...
							<span class="Validators message" style="display:none"></span>
						</ItemTemplate>
					</asp:TemplateColumn>
				</Columns>
		...
		<asp:datagrid id="dgChild" runat="server" CssClass="DataGrid" AutoGenerateColumns="False" Width="100%">
				...
				<Columns>
					...
					<asp:TemplateColumn HeaderText="Tarifa NR">
						...
						<ItemTemplate>
							<asp:TextBox id="txtChildrenFareNR" CssClass="TextBox currency netrate" runat="server" MaxLength="10" Columns="10" Text='<%# DataBinder.Eval(Container, "DataItem.PriceNR") %>'>
							</asp:TextBox>
							<span class="currency"></span>
							<input type="hidden" class="rate" id="varRate" runat="server" value='<%# DataBinder.Eval(Container, "DataItem.price") %>' />		
					...
					<asp:TemplateColumn HeaderText="Tarifa UV">
						...
						<ItemTemplate>
							<asp:TextBox id="txtChildrenFare" CssClass="TextBox currency rate" runat="server" MaxLength="10" Columns="10" Text='<%# DataBinder.Eval(Container, "DataItem.Price") %>'>
							</asp:TextBox>
							<span class="currency"></span>
							<% If Me.IsSupervisor Then%>
			...
			<asp:datagrid id="dgTeen" runat="server" CssClass="DataGrid" AutoGenerateColumns="False" Width="100%">
				...
				<Columns>
					...
					<asp:TemplateColumn HeaderText="Tarifa NR">
						...
						<ItemTemplate>
							<asp:TextBox id="txtTeenFareNR" CssClass="TextBox currency netrate" runat="server" MaxLength="10" Columns="10" Text='<%# DataBinder.Eval(Container, "DataItem.PriceNR") %>'>
							</asp:TextBox>
							<span class="currency"></span>
					...
					<asp:TemplateColumn HeaderText="Tarifa UV">
						...
						<ItemTemplate>
							<asp:TextBox id=txtTeenFare CssClass="TextBox currency rate" runat="server" MaxLength="10" Columns="10" Text='<%# DataBinder.Eval(Container, "DataItem.Price") %>'>
							</asp:TextBox>
							<span class="currency"></span>
							<% If Me.IsSupervisor Then%>
