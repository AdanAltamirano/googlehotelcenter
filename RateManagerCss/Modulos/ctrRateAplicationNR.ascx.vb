...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If TypeOf Me.Page Is FaresCatalogueNR Then
            ...
            If Not Me.IsSupervisor And Me.isUsuarioMixto Then
                Me.txtAdultFare.Attributes.Add("onChange", "javascript:FillPrices('" & CType(Me.Page, FaresCatalogueNR).IdDgAdult & "','txtAdultFare'" & ",'" & Me.txtAdultFare.ClientID & "');CheckValContractUv('" & TextBoxPorcMin.ClientID & "','" & TextBoxPorcMax.ClientID & "','" & txtAdultFareNR.ClientID & "','" & txtAdultFare.ClientID & "','" & lblAdultValMax.ClientID & "','" & lblAdultValMin.ClientID & "','" & PlusTaxProperty & "', '" & EcotasaProperty & "');FillPrices('" & CType(Me.Page, FaresCatalogueNR).IdDgAdult & "','txtAdultFareNR'" & ",'" & Me.txtAdultFareNR.ClientID & "')")
            End If


            Me.txtAdultFareNR.Attributes.Add("onChange", "javascript:FillPrices('" & CType(Me.Page, FaresCatalogueNR).IdDgAdult & "','txtAdultFareNR'" & ",'" & Me.txtAdultFareNR.ClientID & "');CheckValContract('" & TextBoxPorcMin.ClientID & "','" & TextBoxPorcMax.ClientID & "','" & txtAdultFareNR.ClientID & "','" & txtAdultFare.ClientID & "','" & lblAdultValMax.ClientID & "','" & lblAdultValMin.ClientID & "','" & PlusTaxProperty & "', '" & EcotasaProperty & "');FillPrices('" & CType(Me.Page, FaresCatalogueNR).IdDgAdult & "','txtAdultFare'" & ",'" & Me.txtAdultFare.ClientID & "')")
            ...
            Me.txtChildFareNR.Attributes.Add("onChange", "javascript:FillPrices('" & CType(Me.Page, FaresCatalogueNR).IdDgChild & "','txtChildrenFareNR'" & ",'" & Me.txtChildFareNR.ClientID & "')" &
                                                 "; CheckValContract('" & TextBoxPorcMin.ClientID & "','" & TextBoxPorcMax.ClientID & "','" & txtChildFareNR.ClientID & "','" & txtChildFare.ClientID & "','" & lblChildValMax.ClientID & "','" & lblChildValMin.ClientID & "','" & PlusTaxProperty & "', '" & EcotasaProperty &
            ...
            'Validacion de maximos y minimos porcentajes de ganacia de adultos extras y ni�os extras
            'Extra Adult
        ...
        If Page.IsValid Then
            ...
            If Me.m_iFareId = 0 Then
                If SaveNewFare(idRoom, idFare, f1, f2, String.Format("{0} {1}", sroom, rp), sData) Then
                    ...
                    CType(Me.Page, PaginaBase).guardalog("/Pages/FaresCatalogue.aspx", PaginaBase.acciones.Crear, "Se cre� la tarifa de la habitaci�n " & ch.Substring(0, ch.IndexOf("--")) & " de la fecha " & f1 & " a la fecha " & f2 & " con el rateplan " & Me.RatePlanRow.RATECODE, "", sDataPrev, sData)
                    flag = True
                End If
            Else
                If UpdateFare(idRoom, f1, f2, sDataPrev, String.Format("{0} {1}", sroom, rp), sData, scorreo, publish) Then
                    scorreo = (New Util.Utility).GeneraCorreoXslt(sDataPrev, sData)
                    CType(Me.Page, PaginaBase).guardalog("/Pages/FaresCatalogue.aspx", PaginaBase.acciones.Modificar, "Se modific� la tarifa de la habitaci�n " & ch & " de la fecha " & f1last & " a la fecha " & f2last & " con el rateplan " & rp & " su nueva fecha es (o sigue siendo) del " & f1 & " al " & f2 & " el rateplan es (o sigue siendo) " & Me.RatePlanRow.RATECODE, "", sDataPrev, sData)
                    flag = True
                End If
                m_iFareId = 0
            End If
            If Me.txtPromoDescription.HasChanges Then CType(Me.Page, PaginaBase).NotifyContentModification("Tarifa neta de la habitaci�n " & ch & ", y plan tarifario " & Me.RatePlanRow.RATECODE, "Tarifas Netas")
        End If
        Return flag
    End Function
