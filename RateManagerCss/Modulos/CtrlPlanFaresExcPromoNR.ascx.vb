...
Partial Public Class CtrlPlanFaresExcPromoNR
...
#Region "FILL DE LOS NUEVOS DATAGRIDS"
    Private Sub FillDatas()
        ...
        datRestrictions = CType(Me.Page, FaresCataloguePromoNR).GetFareRestrictions
        If Not datRestrictions Is Nothing Then
            For Each dr As DataRow In datRestrictions.Tables(FaresRestrictionsDataExc.FARESRESTRICTIONEXC_TABLE).Rows
                If dr(FaresRestrictionsDataExc.ADULTNUMBER_FIELD) <> ad Then
                    ...
                    drnew("Adults") = ad
                    drnew("Price") = dr(FaresRestrictionsDataExc.EXCADULTFARE_FIELD)
                    drnew("PriceNR") = IIf(dr(FaresRestrictionsDataExc.EXCADULTFARENR_FIELD) Is DBNull.Value, 0, dr(FaresRestrictionsDataExc.EXCADULTFARENR_FIELD))
                ...
                If dr(FaresRestrictionsDataExc.CHILDNUMBER_FIELD) > 0 AndAlso strch.IndexOf("," & dr(FaresRestrictionsDataExc.CHILDNUMBER_FIELD) & ",") = -1 Then
                    ...
                    drnew("Children") = ch
                    drnew("Price") = dr(FaresRestrictionsDataExc.EXCNINIOFARE_FIELD)
                    drnew("PriceNR") = IIf(dr(FaresRestrictionsDataExc.EXCNINIOFARENR_FIELD) Is DBNull.Value, 0, dr(FaresRestrictionsDataExc.EXCNINIOFARENR_FIELD))
                    drnew(FaresRestrictionsDataExc.PKIDRESTRICTION_FIELD) = dr(FaresRestrictionsDataExc.PKIDRESTRICTION_FIELD)
                    dtChildren.Rows.Add(drnew)

                    drnew = dtTeen.NewRow
                    drnew("Teen") = ch
                    drnew("Price") = dr(FaresRestrictionsDataExc.EXTEENFARE_FIELD)
                    drnew("PriceNR") = IIf(dr(FaresRestrictionsDataExc.EXTEENFARENR_FIELD) Is DBNull.Value, 0, dr(FaresRestrictionsDataExc.EXTEENFARENR_FIELD))
                    drnew(FaresRestrictionsDataExc.PKIDRESTRICTION_FIELD) = dr(FaresRestrictionsDataExc.PKIDRESTRICTION_FIELD)
...
#End Region
    ...
    Private Sub FillDataGrid()
        ...
        If Not datRestrictions Is Nothing Then
            If Not datRestrictions.Tables(FaresRestrictionsDataExc.FARESRESTRICTIONEXC_TABLE) Is Nothing AndAlso datRestrictions.Tables(FaresRestrictionsDataExc.FARESRESTRICTIONEXC_TABLE).Rows.Count > 0 Then
                Dim cad As String = Me.FieldException
                If Not String.IsNullOrEmpty(FieldException) Then
                    For I As Integer = 1 To 7
                        Dim CK As CheckBox
                        CK = FindControl("chk" & I.ToString)
                        CK.Checked = False
                ...
                End If
            End If
            combinaciones = datRestrictions.Tables(FaresRestrictionsDataExc.FARESRESTRICTIONEXC_TABLE).Rows.Count
        Else
            combinaciones = 0
        End If
