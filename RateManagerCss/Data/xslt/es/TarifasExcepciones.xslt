      ...
      <body>
        ...
        <xsl:for-each select="TarifasExcepciones/UpdateRatePromo">
          <xsl:if test="position() = 1">
            <span>Tarifas de Promoción </span>
          </xsl:if>
        </xsl:for-each >
        <xsl:for-each select="TarifasExcepciones/UpdateRateNR">
          <xsl:if test="position() = 1">
            <span>Tarifas de Promoción Net Rate</span>
          </xsl:if>
        </xsl:for-each >
        ...
        <xsl:for-each select="TarifasExcepciones/_UpdateRatePromo">
          <xsl:if test="position() = 1">
            <span>Tarifas de Promoción, Actualizado </span>
          </xsl:if>
        </xsl:for-each >
        <xsl:for-each select="TarifasExcepciones/_UpdateRateNR">
          <xsl:if test="position() = 1">
            <span>Tarifas Netas de Promoción, Actualizado </span>
          </xsl:if>
        </xsl:for-each >
        <table border="1" cellspacing="0" cellpadding="0">
          <xsl:for-each select="TarifasExcepciones/_UpdateRatePromo">
