...
function ldDiff(){
  ...
  if(bar&amp;&amp;total>0){
    ...
    if(cnt){cnt.textContent=total+' field'+(total!==1?'s':'')+' modified';}
  }
}
document.readyState!=='loading'?ldDiff():document.addEventListener('DOMContentLoaded',ldDiff);
      ...
      <body>
        <div class="logdiff">
          ...
          <!-- Summary bar (visible only when there are changes) -->
          ...
          <!-- Detect DELETED state: "before" exists but "after" does not -->
            ...
            <div>
              ...
              <xsl:choose>
                <xsl:when test="$isDeleted">
                  <div class="ld-head deleted">
                    <span class="ld-badge deleted">&#10006; DELETED</span>
                    <xsl:if test="Tarifas/UpdateRate">Deleted Rates</xsl:if>
                    <xsl:if test="Tarifas/UpdateRateByDay"> &#183; Rates by Day</xsl:if>
                    <xsl:if test="Tarifas/UpdateRateNR"> &#183; Net Rates</xsl:if>
                  </div>
                </xsl:when>
                <xsl:otherwise>
                  <div class="ld-head before">
                    <span class="ld-badge before">&#9664; BEFORE</span>
                    <xsl:if test="Tarifas/UpdateRate">Rates</xsl:if>
                    <xsl:if test="Tarifas/UpdateRateByDay"> &#183; Rates by Day</xsl:if>
                    <xsl:if test="Tarifas/UpdateRateNR"> &#183; Net Rates</xsl:if>
                  </div>
                </xsl:otherwise>
              ...
              <xsl:if test="Tarifas/UpdateRate">
                <table class="ld-tbl" id="ld-b-r">
                  <thead><tr>
                    <th>Room/Rate Plan</th><th>Start Date</th><th>End Date</th>
                    <th>Price</th><th>Child P.</th><th>Teen P.</th>
                    <th>Adult Extra P.</th><th>Child Extra P.</th><th>Teen Extra P.</th>
                  </tr></thead>
                  <tbody>
                ...
                <table class="ld-tbl ld-tbl-rules" id="ld-b-r2">
                  <thead><tr>
                    <th>Default Rules</th><th>Pers.</th><th>Max. Adults</th><th>Min. Adults</th>
                    <th>Max. Children</th><th>Extra Persons</th><th>Adv. Booking</th>
                    <th>Max. days</th><th>Min. days</th><th>Arrivals</th>
                    <th>Exceptions</th><th>Wait List</th><th>Window Start</th>
                    <th>Window End</th><th>Promo Disc.</th>
                  </tr></thead>
                  <tbody>
                    <xsl:for-each select="Tarifas/UpdateRate">
                      <tr>
              ...
              <xsl:if test="Tarifas/UpdateRateByDay">
                <table class="ld-tbl" id="ld-b-d">
                  <thead><tr>
                    <th>Room/Rate Plan</th><th>Start Date</th><th>End Date</th>
                    <th>Adult Price</th><th>Adult ext.P.</th><th>Child P.</th><th>Child ext.P.</th>
                  </tr></thead>
                  <tbody>
              ...
              <xsl:if test="Tarifas/UpdateRateNR">
                <table class="ld-tbl" id="ld-b-n">
                  <thead><tr>
                    ...
                    <th>Price</th><th>Child P.</th><th>Teen P.</th>
                    <th>Adult Extra P.</th><th>Child Extra P.</th><th>Teen Extra P.</th>
                    <th>Price NR</th><th>Child NR</th><th>Teen NR</th>
                    <th>Adult Extra NR</th><th>Child Extra NR</th><th>Teen Extra NR</th>
                  </tr></thead>
                  <tbody>
          ...
          <xsl:if test="(Tarifas/UpdateRate or Tarifas/UpdateRateByDay or Tarifas/UpdateRateNR) and (Tarifas/_UpdateRate or Tarifas/_UpdateRateByDay or Tarifas/_UpdateRateNR)">
            <div class="ld-sep">
              ...
              <div class="ld-sep-txt">&#9660; UPDATED TO</div>
              <div class="ld-sep-line"></div>
            </div>
          </xsl:if>
          ...
          <xsl:if test="Tarifas/_UpdateRate or Tarifas/_UpdateRateByDay or Tarifas/_UpdateRateNR">
            <div class="ld-card">
              <div class="ld-head after">
                ...
                <xsl:if test="Tarifas/_UpdateRate">Rates</xsl:if>
                <xsl:if test="Tarifas/_UpdateRateByDay"> &#183; Rates by Day</xsl:if>
                <xsl:if test="Tarifas/_UpdateRateNR"> &#183; Net Rates</xsl:if>
              </div>
          ...
          <div id="ld-legend" class="ld-legend">
            <span><span class="ld-dot" style="background:#fee2e2;border:1px solid #fca5a5;"></span> Previous value (strikethrough)</span>
            <span><span class="ld-dot" style="background:#f0fdf4;border:1px solid #4ade80;"></span> Updated value</span>
          </div>
  ...
  <xsl:template name="FormatDate">
    ...
    <xsl:choose>
      <xsl:when test="$mo = '01'">Jan</xsl:when><xsl:when test="$mo = '02'">Feb</xsl:when>
      <xsl:when test="$mo = '03'">Mar</xsl:when><xsl:when test="$mo = '04'">Apr</xsl:when>
      <xsl:when test="$mo = '05'">May</xsl:when><xsl:when test="$mo = '06'">Jun</xsl:when>
      <xsl:when test="$mo = '07'">Jul</xsl:when><xsl:when test="$mo = '08'">Aug</xsl:when>
      <xsl:when test="$mo = '09'">Sep</xsl:when><xsl:when test="$mo = '10'">Oct</xsl:when>
      <xsl:when test="$mo = '11'">Nov</xsl:when><xsl:when test="$mo = '12'">Dec</xsl:when>
    </xsl:choose>
