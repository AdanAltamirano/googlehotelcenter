...
export default {
    ...
    methods:{
        showLoader() {
            this.loader = this.$loading.show({ color: '#007bff', height: 64, width: 64, isFullPage: false });
        },
        hideLoader() {
            this.loader.hide();
        },
        ...
        loadDailyRate(dayRate){
             RatesService.getByDay(this.$appConfig.session.hotelId, this.dayRate.rateId, this.dayFormatted)
            .then((response) => {
