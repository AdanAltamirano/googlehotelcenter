<template>
	<div class="mt-4">
		<h5 class="text-info" style="cursor:pointer" v-b-toggle.roomsCollapse>
		...
		<div v-else>
			<b-collapse visible id="roomsCollapse">
				<table class="fixed_header" style="border-collapse:separate !important; border-spacing:1px !important;">
					...
					<tbody>
						<tr class="tr-config" v-for="room in rooms" :key="room">
							<td>{{room.idRoomIp}}</td>
							<td>
								<b-form-input v-model="room.idRoomArpon" ></b-form-input>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<div v-else>
					<b-button v-if="showButton" variant="primary" @click="SaveRoomsArpon">{{$t('Save')}}</b-button>
				</div> 
			</b-collapse>
		</div>
	</div>
...
export default {
	...
	methods:{
		...
		GetRoomsArpon(){
			this.loading = true;
			ArponService.GetRoomsByHotelId(this.hotelId)
			.then(response =>{
		...
		SaveRoomsArpon(){
			...
			ArponService.SaveRoomsArpon(this.hotelId,this.rooms)
			.then(response => {
				console.log(response);
				this.callApi = false;
				this.showButton = true;
				this.$appAlert(this.success(this.$t("Rooms Saved")));
			})
			.catch(error => {
				console.log(error);
				this.callApi = false;
