            ...
            <div class="card-body">
                ...
                <div v-if="result.paymentWay == 0">
                    <address v-if="result.bankDepositDetails">
                        Total: <strong>{{result.bankDepositDetails.total}}&nbsp;{{result.bankDepositDetails.currency}}</strong>
                        <br>
                        {{$t('Reference')}}: <strong>{{result.bankDepositDetails.reference}}</strong>
                    </address>
                    <b-alert show v-if="result.status == 4" variant="danger">
                        {{$t('Please confirm by email once payment is made to')}} 
                        <a href="mailto:reservas@internetpowerhotel.com">{{$t('Collection dept')}}</a>
                    </b-alert>
                </div>
                ...
                <div v-if="result.paymentWay == 1">
                    ...
                    <address class="mt-1">
                        {{$t('Authorization number')}}: <strong>NA</strong>
                    </address>
                </div>
                <!--FIN PAGO EN LINEA-->
                <!--PAGO EN EL HOTEL-->
                <div v-if="result.paymentWay == 2">
                    <address v-if="result.customer.cardDetails">
                        ...
                        <div v-if="!ccDetails.isSuccess" class="d-flex">
                            <span class="mt-2">
                                {{$t('Card number')}}: <strong>{{result.customer.cardDetails.number}}</strong>
                            </span>
                            <b-button 
                            ...
                            v-show="showBtn" 
                            variant="link">{{$t('View card data')}}</b-button>
                        </div>
                        <div v-if="ccDetails.isSuccess">
                            <address class="mt-2">
                                {{$t('Name')}}: <strong>{{ccDetails.owner}}</strong>
                                <br>
                                {{$t('Type')}}: <strong>{{ccDetails.cardType}}</strong>
                                <br>
                                {{$t('Card number')}}: <strong>{{ccDetails.number}}</strong>
                                <br>
                                {{$t('Expiration date')}}: <strong>{{ccDetails.monthExpiration}}/{{ccDetails.yearExpiration}}</strong>
                                <br>
                                {{$t('Security code')}}: <strong>{{ccDetails.cvv}}</strong>
                            </address>
                        </div>
                    </address>
                    <b-alert
    ...
    computed: {
        paymentWay() {
            ...
            switch(this.result.paymentWay) {
                case 0: txt = this.$t('Bank deposit');break;
                case 1: txt = this.$t('Online payment');break;
                case 2: txt = this.$t('Payment at the hotel');break;
            }; return txt;
        },
        description_dismiss() {
            let txtTime = '';
            ...
            {
                seconds -= minutes * 60;
                txtTime += `${minutes} ${minutes > 1 ? `${this.$t('minute')}s` : this.$t('minute')}`; 
            }
            txtTime += `${(minutes > 0 && seconds > 0) ? ', ' : ''}`;
            if (seconds > 0)
                txtTime += `${seconds} ${seconds > 1 ? `${this.$t('second')}s` : this.$t('second')}`;
