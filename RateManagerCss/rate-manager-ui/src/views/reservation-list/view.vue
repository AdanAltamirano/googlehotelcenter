    ...
    <b-container fluid>
      ...
      >
        <template slot="idconfirmNumber" slot-scope="data">
          <b-link
            target="_blank"
            :href="'/rate-manager-ui/dist/reservation-details.aspx?qs=' + data.item.id"
          >{{data.item.confirmNumber}}</b-link>
        </template>
        <template slot="client" slot-scope="data">
          <span
            v-tooltip="data.value"
...
export default {
  ...
  mounted() {
    this.$root.$on("table-result", val => {
      this.result = val;
      console.log("Mounted result " + this.result);
      this.excelFormat();
    });
  },
  created() {
  ...
  methods: {
    ...
    defaultSearch() {
      ...
      const s = `Status eq 1 and ReservationDate gt ${this.$moment(
        x.start
      ).format("YYYY-MM-DD")} 
