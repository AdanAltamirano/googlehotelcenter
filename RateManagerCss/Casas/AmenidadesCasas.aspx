<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="AmenidadesCasas.aspx.vb" Inherits="RateManager.AmenidadesCasas" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <script>  
        ...
        function deleteCategory(id) {
            ...
            function(){               
                ...
                jQuery.ajax({
                    type: "GET",
                    url: "Crud.ashx",
                    data: { "MethodName": "deleteCategory", "id": id },
        ...
        function deleteAmenity(id) {
            ...
            swal({
                title: "¿Esta seguro de eliminar la amenidad seleccionada ?",
                text: "Una vez eliminado no podrá ser recuperado!",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
            ...
            function () {
                ...
                jQuery.ajax({
                    type: "GET",
                    url: "Crud.ashx",
                    data: { "MethodName": "deleteAmenity", "id": id },
                    success: function (response) {
                        //var data = JSON.parse(response);
                        console.log(response)

                        if (response.Error == "False") {
                            swal("Eliminado!", "La Amenidad a sido eliminada.", "success");
                            getAminitiesByIdCategory(idCat);
                        }
                    }
                });
            });
        ...
        function getAllCategories() {
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    $("#listado_categorias").prop('selectedIndex', 0);
        ...
        function getAminitiesByIdCategory(id_cat) {
            jQuery.ajax({
                type: "GET",
                url: "Crud.ashx",
                data: { "MethodName": "GetAmenitiesByCategory", "id": id_cat },
                success: function (data) {
                    //var data = JSON.parse(response);
                    console.log("Amenidades ", data);
                    ...
                    if (data.results.length != 0) {
                        ...
                        $.each(data.results, function (i, item) {
                            trHTML += '<tr id=' + item.id_dictionary + '><td>' + item.name + '</td><td class="options">' +
                                        '<input type="button" id="btn_delete" class="btn btn-danger btn_crud" value="eliminar" onclick="deleteAmenity(' + item.id + ')"  />' +
                                        '<input type="button" id="btn_update" class="btn btn-warning btn_crud" value="editar" onclick="updateCategory(' + item.id + ',' + item.id_dictionary + ',' + item.idCategoriaAmenidad + ')" />' +
                                        '</td></tr>';
                        });

                        $("#listado_categorias").prop('selectedValue', id_cat);
                    } else {

                        swal("No hay amenidades con esa categoría.", "Ingresa una amenidad con la categoría seleccionada en caso de ser necesario.")
                    }
                    
                    $('#amenities_cat').html(trHTML);
                }
            });
        }

        function getAmenitiesByIdCat() {
            getAminitiesByIdCategory(idCat);
        }
        ...
        function errorDataBase() {
            swal("Error al insertar en la bd.", "Debuguear...", "error");
        }
    ...
    <form id="form1" class="form-inline" runat="server">
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <div>
                    <h4>Agregar Amenidades</h4>
                    <div class="form-group">
                        <uc2:CtrlIdioma runat="server" ID="ctrlIdiomaAmen" />
                    </div>
                    <!-- Tracala para guardar los datos en inputs invisibles -->
                    <input id="id_amen_d" type="hidden" runat="server" value ="0"/>
                    <input id="id_dic_d" type="hidden" runat="server" value ="0"/>

                    <select class="form-control" id="listado_categorias">
                    </select>

                    <input id="Hidden1" type="hidden" runat="server" />
                    <asp:Button class="btn btn-primary btn_crud" data-toggle="tooltip" data-placement="top" title="Recuerda ingresar el nombre en Inglés y Español." id="agregar_amen" runat="server" Text="Agregar" />
                    <asp:Button id="btn_actualizar" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Recuerda ingresar el nombre en Inglés y Español." Text="Actualizar" runat="server"/>
                    <button id="btn_cancel" class="btn btn-primary" onclick="cancelUpdateCat()">Cancelar</button> 
                    <input type="button" class="btn btn-success btn_crud_mainA" id="ver_amen" value="Ver" onclick="getAmenitiesByIdCat()"/> 
                </div>
                <hr />
                <div class="panel panel-info">
                    <div class="panel-heading">Amenidades registradas</div>
                    <div class="panel-body">
                        <table class="table" id="amenities_cat">
