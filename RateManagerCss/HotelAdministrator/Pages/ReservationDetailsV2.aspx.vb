...
#End Region
    ...
    Private Sub hideByHotel()
        If MyBase.IsHotel Or MyBase.IsUsuarioHotel Then
            ...
            Me.lblPhoneH.Visible = False
            Me.lblPhoneW.Visible = False

            Me.lblAddress.Visible = False
            Me.lblZipCode.Visible = False
            Me.lblCustCity.Visible = False
            Me.lblCustCounty.Visible = False
            Me.lblCustCountry.Visible = False

            lblCCNumber.Visible = False
            Me.lblCCType.Visible = False

            tdCostoHotel.Visible = False
            hideCost.Visible = False
            hideDep.Visible = False

            Me.lblEmail.Visible = False

            Me.lblEPhoneH.Visible = False
            Me.lblEPhoneW.Visible = False
            Me.lblECode.Visible = False
            Me.lblCiudad.Visible = False
            Me.lblEstado.Visible = False
            Me.lblPais.Visible = False
            Me.lblCCNu.Visible = False
            lnkShowCC.Visible = False
            Me.lblCCTitle.Visible = False
            Me.lblCostos.Visible = False
            Me.lblEAddress.Visible = False
    ...
    Private Sub LoadResources()
        ...
        lblCheckOut.Text = PortalCulture.GetString("01586", True)
        earlyOut.Text = PortalCulture.GetString("01586")
        lblConfirmModify.Text = PortalCulture.GetString("01586")
        Modify.Value = PortalCulture.GetString("M000228")
        ...
        Me.lblEAddress.Text = PortalCulture.GetString("M000076")
    ...
    Private Function getData(ByVal ID As Integer)
        ...
        If Not dsReservaciones Is Nothing AndAlso dsReservaciones.Tables(dsReservaciones.RESERVA_TABLE).Rows.Count > 0 Then
            With dsReservaciones.Tables(dsReservaciones.RESERVA_TABLE).Rows(0)
                ...
                If .Item(dsReservaciones.FIELD_STATUS) = "1" AndAlso .Item(dsReservaciones.FIELD_Guaranteed) = "0" AndAlso Not .Item(dsReservaciones.FIELD_IsNetRateUv) Then
                    Me.lblStatus.Text = Me.lblStatus.Text & " " & PortalCulture.GetString("1529")
                End If

                Dim dar As New dsReservTableAdapters.SalidasAnticipadasTableAdapter
                Dim dsr As New dsReserv.SalidasAnticipadasDataTable

                dar.FillByIdReservacion(dsr, Me.IdReservacion)
                If dsr.Rows.Count > 0 Then
                    If dsr.Rows(0)("noShow") = True Then
                        lblStatus.Text &= " / No Show"
                    Else
                        lblStatus.Text &= " / Early Check-Out"
                    End If
                End If

                dar.FillByOldChecOut(dsr, Me.IdReservacion)
                lblOldOut.Visible = False
                lblEOldOut.Visible = False
                If dsr.Rows.Count > 0 Then
                    lblOldOut.Text = CDate(dsr.Rows(0)("oldCheckOut")).ToString("dd/MMM/yyyy")
                    lblOldOut.Visible = True
                    lblEOldOut.Visible = True
                End If
        ...
        Me.lblInDate.Text = CDate(.Item(dsReservaciones.FIELD_CHECKIN)).ToString("dd/MMM/yyyy")
        ...
        If Not Session(AppSettings("RestTarjetas")) Is Nothing AndAlso Session(AppSettings("RestTarjetas")) = "1" Then
            lnkShowCC.Visible = False
        Else
            If (IsSupervisor Or Not isNR) And Not .IsNull(dsReservaciones.FIELD_NUMEROCC) Then
    ...
    Private Sub hplModifyRes_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles hplModifyRes.Click
        ...
        If earlyOut.Checked Then
            Try
                Dim dateAux As DateTime = DateTime.Parse(txtCheckOut.Text)

                If DateDiff(DateInterval.Day, CDate(lblInDate.Text), dateAux) <= 0 Then
                    lblErrorPop.Text = PortalCulture.GetString("00664")
                    isValidDate = False
                End If
                If DateDiff(DateInterval.Day, CDate(lblOut.Text), dateAux) >= 0 Then
                    lblErrorPop.Text = String.Format(PortalCulture.GetString("01585"), lblOut.Text)
                    isValidDate = False
                End If
            Catch ex As Exception
                isValidDate = False
                lblErrorPop.Text = PortalCulture.GetString("M000197")
            End Try
        End If

        If isValidDate Then
            Dim conn As New SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("ozhotelesConnectionString").ConnectionString)
            ...
            Try
                ...
                Dim dar As New dsReservTableAdapters.ReservacionesTableAdapter
                Dim dsr As New dsReserv
                Dim dr As dsReserv.ReservacionesRow

                dar.Connection = conn
                dar.Transaction = trans



                dr = dsr.Reservaciones.NewReservacionesRow

                dr.idReservacion = Request.QueryString("qs")

                dsr.Reservaciones.AddReservacionesRow(dr)
                dsr.Reservaciones.AcceptChanges()

                If noShow.Checked Then
                    dr.CheckOut = DateTime.Parse(lblInDate.Text).AddDays(1)
                Else
                    dr.CheckOut = txtCheckOut.Text
                End If
                dar.Update(dsr.Reservaciones)

                Dim das As New dsReservTableAdapters.SalidasAnticipadasTableAdapter
                Dim dss As New dsReserv
                Dim drs As dsReserv.SalidasAnticipadasRow
                Dim dtFecha As DateTime = DateTime.Now

                das.Connection = conn
                das.Transaction = trans

                drs = dss.SalidasAnticipadas.NewSalidasAnticipadasRow
                If earlyOut.Checked Then
                    drs.earlyOut = 1
                    drs.noShow = 0
                End If
                If noShow.Checked Then
                    ...
                    drs.earlyOut = 0
                End If
                drs.idReservacion = Request.QueryString("qs")
                drs.oldCheckOut = DateTime.Parse(lblOut.Text)
                drs.fechaRegistro = dtFecha
                das.Insert(drs.idReservacion, drs.noShow, drs.earlyOut, drs.oldCheckOut, drs.fechaRegistro)

                trans.Commit()
            Catch ex As Exception
                If trans IsNot Nothing Then
                    trans.Rollback()
                    lblErrorPop.Text = PortalCulture.GetString("00666")
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "SelectDefault2", "<script>ShowResStatus();ShowError('" & txtCheckOut.Text & "');" & "</scr" + "ipt>")
                End If
        ...
        Else
            ' Page.ClientScript.RegisterStartupScript(Me.GetType(), "SelectDefault", "<script>;" & "</scr" + "ipt>")
            Page.ClientScript.RegisterStartupScript(Me.GetType(), "SelectDefault2", "<script>ShowResStatus();ShowError('" & txtCheckOut.Text & "');" & "</scr" + "ipt>")
        End If
