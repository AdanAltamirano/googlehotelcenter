...
Imports System.Data
Imports XCrypt
Partial Public Class CtrlHotelItinerary
    ...
    Private Function GetStatus(ByVal idstatus As Integer, ByVal source As String, ByVal IdCorporativoPortal As Integer, ByVal isNetRateUv As Boolean, ByVal idAsociacion As Integer) As String
        ...
        Select Case idstatus
            Case 1
                ...
                If (source = "UNI") Then
                    If (Not isNetRateUv) Or CType(Me.Page, PaginaBase).IsSupervisor Then
                        'Me.btnCancel.Visible = True
                    End If
                ElseIf (source = "HTL") Or CType(Me.Page, PaginaBase).IsSupervisor Then 'Or ((Me.IsUsuarioNivelHotel Or Me.isUserChain) AndAlso source = "HTL") Then
                    'Me.btnCancel.Visible = True
                ElseIf CType(Me.Page, PaginaBase).isUserChain AndAlso source = "POR" AndAlso IdCorporativoPortal <> -1 AndAlso CType(Me.Page, PaginaBase).IdCorporativoUserChain = IdCorporativoPortal Then
                    'Me.btnCancel.Visible = True
                End If
                If source = "POR" And isNetRateUv And Not CType(Me.Page, PaginaBase).IsSupervisor Then
                    Me.btnCancel.Visible = False
                End If
                If source = "POR" And CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation Then
                    If idAsociacion = CType(Me.Page, PaginaBase).IdAsociation Then
                        'Me.btnCancel.Visible = True
                    Else
                        Me.btnCancel.Visible = False
                    End If
            ...
            Case 4
                lblNoCanc.Visible = False
                Me.lblNoCancelacion.Visible = False
                If CType(Me.Page, PaginaBase).IsSupervisor Or source = "HTL" Then '((Me.IsUsuarioNivelHotel Or Me.isUserChain) AndAlso source = "HTL") Then
                    'Me.btnCancel.Visible = True
    ...
    Private Sub LoadResources()
        ...
        Me.lblESource.Text = PortalCulture.GetString("M000324", True) & " "
        ...
        If Me.lblTotalUV.Text.Trim().Length > 0 Then
            ...
            ElseIf CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation Then
        ...
        For Each drRooms As resHotelDisplay.RoomRow In xml.Room
            ...
            For i As Int32 = 0 To dr.Length - 1
                ...
                For Each drRate As resHotelDisplay.RateRow In drateArr
                    ...
                    If Not CType(Me.Page, PaginaBase).IsSupervisor AndAlso Not CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation Then
                    ...
                    If Not lstTarifas.Contains(sItem) Then
                        ...
                        If (dPrecio <> dImporte) Then
                            sHtmlRate &= String.Format("<tr><td>{0}</td> <td style='padding-left:6px;'>{1} {2}</td></tr> ", _
                                                        sDias, CType(Me.Page, PaginaBase).FCurrency(dImporte, 2), IIf(dImporte > 0, money, ""))
                            dPrecio = dImporte
                            sDias = ""
                            sHab = ""
                            sComa = ""
    ...
    Private Sub GetDataWS(ByVal nores As String)
        Try
            ...
            If Not xml Is Nothing AndAlso xml.Reservation.Rows.Count > 0 Then
                ...
                Dim stblRooms As String = "<table border=0 cellpadding=0 width=99%>"
                Dim sHab As String
                stblRooms &= String.Format("<tr class='litletitle'><td width='8%'>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>" & PortalCulture.GetString("M000339") & "</td><td width='5%'>{0}</td><td width='5%'>{1}</td><td width='5%'>{2}</td><td width='5%'>{3}</td><td width='5%'>{4}</td></tr>", _
                                          PortalCulture.GetString("00621"), PortalCulture.GetString("M000320"), _
                                          PortalCulture.GetString("00428"), PortalCulture.GetString("00429"), PortalCulture.GetString("01367"))
                Dim i As Byte = 1
                For Each drRoom As resHotelDisplay.RoomRow In xml.Room
                    '  hab = String.Format("{0}" drRoom.NameRoom)
                    sHab = String.Format(PortalCulture.GetString("01396"), i)
                    stblRooms &= String.Format("<tr class='littletext'><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8}</td><td>{9}</td></tr>", _
                                         sHab, String.Concat(drRoom.TravelerFirstName.ToString(), " ", drRoom.TravelerLastName.ToString()), _
                                         drRoom.NameRoom, xml.Reservation(0).RatePlan & " " & drRoom.RatePlanName, IIf(drRoom.Preferences = "", "--", drRoom.Preferences), drRoom.Adults, drRoom.Children, drRoom.ExtraAdults, drRoom.ExtraChildren, IIf(drRoom.ChildrenAges = "", "--", drRoom.ChildrenAges), "")
                    i += 1
                Next
                ...
                If Not isNetRateUV Then 'NORMAL
                    ...
                    Me.lblTotal.Text = CType(Me.Page, PaginaBase).FCurrency(dr.Total, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblImpuestos.Text = CType(Me.Page, PaginaBase).FCurrency(dr.Taxes, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)

                    If CType(Me.Page, PaginaBase).IsSupervisor Or CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation Then
                        'mostrar la comision y el comisionfee
                        Me.lblTotal.Text = CType(Me.Page, PaginaBase).FCurrency((dr.Total + dr.ServiceFee), 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                        Me.lblImpuestos.Text = CType(Me.Page, PaginaBase).FCurrency((dr.Taxes + dr.ServiceFee), 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    End If
                    Me.lblTotalH.Text = CType(Me.Page, PaginaBase).FCurrency(dr.Total, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblCosto.Text = CType(Me.Page, PaginaBase).FCurrency(((dr.Total - dr.Taxes) / (DateDiff(DateInterval.Day, CDate(dr.CheckInDate), CDate(dr.CheckOutDate)))), 2) & " " & dr.Money
                    ...
                    End If
                Else ' NETRATE
                    aNRPolicies.Visible = CType(Me.Page, PaginaBase).IsSupervisor Or CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation
                    lblTipoReservacion.Text = PortalCulture.GetString("01125")
                    tdCostoUV.Visible = CType(Me.Page, PaginaBase).IsSupervisor Or CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation
                    tdTextHotel.Visible = CType(Me.Page, PaginaBase).IsSupervisor Or CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation
                    tdTextUnivisit.Visible = CType(Me.Page, PaginaBase).IsSupervisor Or CType(Me.Page, PaginaBase).IsUsuarioHotelAssociation

                    'HOTEL
                    Me.lblTotal.Text = CType(Me.Page, PaginaBase).FCurrency(totalNR, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblImpuestos.Text = CType(Me.Page, PaginaBase).FCurrency(taxesNR, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblTotalH.Text = CType(Me.Page, PaginaBase).FCurrency(totalNR, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblCosto.Text = CType(Me.Page, PaginaBase).FCurrency(((totalNR - taxesNR) / (DateDiff(DateInterval.Day, CDate(dr.CheckInDate), CDate(dr.CheckOutDate)))), 2) & " " & dr.Money
                    If xml.Reservation(0)("plustax").ToString.ToLower = "false" Then
                        lbl2.Visible = True
                        lblImpuestos.Visible = True
                        lblEImpuesto.Visible = True
                    ...
                    'UNIVISIT
                    Me.lblTotalUV.Text = CType(Me.Page, PaginaBase).FCurrency(dr.Total, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblImpuestosUV.Text = CType(Me.Page, PaginaBase).FCurrency(dr.Taxes, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblTotalHUV.Text = CType(Me.Page, PaginaBase).FCurrency(dr.Total, 2) & " " & If(dr.IsMoneyNull, "", dr.Money)
                    Me.lblCostoUV.Text = CType(Me.Page, PaginaBase).FCurrency(((dr.Total - dr.Taxes) / (DateDiff(DateInterval.Day, CDate(dr.CheckInDate), CDate(dr.CheckOutDate)))), 2) & " " & dr.Money
                ...
                If xml.Reservation(0).AgeNombre <> "" Then
                    ...
                    strAgency &= "<tr>"
                    strAgency &= "<td vAlign='top'>"
                    strAgency &= "<p>" & PortalCulture.GetString("M0BT0000322").Substring(0, 8) & " : " & IIf(xml.Reservation(0).PorcPor, xml.Reservation(0).PorcPor, "") & "%</p>"
                    strAgency &= "                              </td>"
                    strAgency &= "</tr>"
                    strAgency &= "</table>"
            ...
            If xml.Reservation(0).DepositAmount > 0 AndAlso xml.Reservation(0).Status = 1 Then
                'fue reservación por depósito 

                If Not dsrs Is Nothing AndAlso dsrs.Tables(0).Rows.Count > 0 Then
                    lblDeposito.Text = String.Format(PortalCulture.GetString("00811"), CType(Me.Page, PaginaBase).FCurrency(dsrs.Tables(0).Rows(0).Item("dep_monto"), 2), Format(dsrs.Tables(0).Rows(0).Item("dep_moneda")))
            ...
            If xml.Reservation(0).ReservedByPayPal = False AndAlso xml.Reservation(0).OnRequest = False AndAlso Not IsPaymentOnline AndAlso Not IsDineroMail Then
                ...
                If deposit > 0 Then
                    Me.lblpay.Text = PortalCulture.GetString("01841")
                    ...
                    'xml.Reservation(0).DepositReference, deposit & " " & xml.Reservation(0).Money)
                    'Me.lblStatus.Text = PortalCulture.GetString("01823")
                End If
            ...
            ElseIf IsPaymentOnline Then
                ...
                '                tc =CType(Me.Page, PaginaBase).FCurrency(ds.Tables(0).Rows(0).Item("TipoDeCambio"), 2)
                '            End If
                '        End If
                ...
                Me.lblpay.Text = String.Format(PortalCulture.GetString("01388"), If(xml.Reservation(0).Status = 1, xml.Reservation(0).NoAutorizacion, "N/A"), xml.Reservation(0).PaymentReference)
                If hr Then
                    Me.lblpay.Text &= String.Format("<br/>{0}: {1} {2}", PortalCulture.GetString("00037"), deposit.ToString("#,###,##0.00"), xml.Reservation(0).paymentCurrency)
            ...
            Else
                Me.lblpay.Text = ""
            End If
            lblpay.Text = "<span class='MiddleText'>" & lblpay.Text & "</span>"
        Catch ex As Exception
            Trace.Write(" ... " & ex.StackTrace)
            Response.Redirect(CType(Me.Page, PaginaBase).UrlPage(PaginaBase.pages.ReservationList))
    ...
    Private Function cargausuarios() As Boolean
        ...
        With New UsuarioHotelFacade
            ds = .GetUserListByIdUser(CType(Me.Page, PaginaBase).Usuario)
            isShow = True
            If Not ds Is Nothing AndAlso ds.Tables(0).Rows.Count > 0 Then
    ...
    Public Function getData(ByVal ID As Integer)
        ...
        If Not dsReservaciones Is Nothing AndAlso dsReservaciones.Tables(dsReservaciones.RESERVA_TABLE).Rows.Count > 0 Then
            With dsReservaciones.Tables(dsReservaciones.RESERVA_TABLE).Rows(0)
                ...
                Me.CanShowConfirmButton = (CType(Me.Page, PaginaBase).IsSupervisor AndAlso ((.Item(dsReservaciones.FIELD_STATUS) = 3 AndAlso .Item("NoCancelacion").ToString().ToUpper().StartsWith("CXSYS")) OrElse .Item(dsReservaciones.FIELD_STATUS) = 4) AndAlso pagoData IsNot Nothing AndAlso pagoData.Rows.Count > 0 AndAlso .Item("IsOnlinePayment"))
                ...
                If .Item(dsReservaciones.FIELD_STATUS) = 3 AndAlso Not .Item("DeadLine") Is DBNull.Value AndAlso .Item("DeadLine") = 1 Then
                    If Not .Item("PenaltyAmount") Is DBNull.Value Then
                        Me.PenaltyAmount = CType(.Item("PenaltyAmount"), Decimal)
                    End If
                End If

                If .Item(dsReservaciones.FIELD_STATUS) = "1" And .Item(dsReservaciones.FIELD_Guaranteed) = "0" Then
                    Me.lblStatus.Text = Me.lblStatus.Text & " " & PortalCulture.GetString("1529")
                End If
                ...
                If Not .IsNull(dsReservaciones.FIELD_RAwayAdult) Then
                    ...
                    If Not .IsNull(dsReservaciones.FIELD_PriceRAwayAdult) Then
                        Me.lblPriceAdult.Text = CType(Me.Page, PaginaBase).FCurrency(.Item(dsReservaciones.FIELD_PriceRAwayAdult), 2)
                    End If
                End If
                If Not .IsNull(dsReservaciones.FIELD_RAwayChild) Then
                    ...
                    If Not .IsNull(dsReservaciones.FIELD_PriceRAwayChild) Then
                        Me.lblPriceChild.Text = CType(Me.Page, PaginaBase).FCurrency(.Item(dsReservaciones.FIELD_PriceRAwayChild), 2)
                    End If
                End If
                If Not .IsNull(dsReservaciones.FIELD_RAwayCrib) Then
                    ...
                    If Not .IsNull(dsReservaciones.FIELD_PriceRAwayCrib) Then
                        Me.lblPriceCrib.Text = CType(Me.Page, PaginaBase).FCurrency(.Item(dsReservaciones.FIELD_PriceRAwayCrib), 2)
                    End If
                End If
                If .Item(dsReservaciones.FIELD_WizcomPassOn).ToString <> "" Then
                    ...
                    Dim dv As DataView
                    ds.ReadXml(Server.MapPath(CType(Me.Page, PaginaBase).GeRequestApplicationPath("/Data/RFCode.xml")))
                ...
                If Not .IsNull(dsReservaciones.FIELD_NUMEROCC) Then
                    ...
                    Else
                        If (Not CType(Me.Page, PaginaBase).IsSupervisor And isNR = True) Or Not lblCCExp.Visible Then
                            lblCCNumber.Text = ccN
                        Else
                ...
                If Not Session(AppSettings("RestTarjetas")) Is Nothing AndAlso Session(AppSettings("RestTarjetas")) = "1" Then
                    ...
                    End If
                Else
                    If Not CType(Me.Page, PaginaBase).IsSupervisor And isNR = True Then
                        lblCCcvNumber.Text = "XXX"
                    Else
                        lblCCcvNumber.Text = IIf(.IsNull(dsReservaciones.FIELD_DIGITOCC), " -", .Item(dsReservaciones.FIELD_DIGITOCC))
                    End If
                ...
                Else
                    If (CType(Me.Page, PaginaBase).IsSupervisor Or Not isNR) And Not .IsNull(dsReservaciones.FIELD_NUMEROCC) Then
                ...
                Else
                    If Not dsReservaciones.Tables(ReservaDatos.MONEDA_TABLE) Is Nothing AndAlso dsReservaciones.Tables(ReservaDatos.MONEDA_TABLE).Rows.Count > 0 Then
                        If Not IsDBNull(dsReservaciones.Tables(ReservaDatos.MONEDA_TABLE).Rows(0).Item(ReservaDatos.FIELD_MONEDA)) Then
                ...
                If .Item("source").ToString.ToUpper = "CCT" Then
                    Me.lblSource.Text = "Call Center"
                    Me.divClientportal.Style.Add("display", "block")
                ...
                ElseIf .Item("source").ToString.ToUpper.Trim = "POR" AndAlso .Item("SourceCode").ToString.ToUpper.Trim = "HB" AndAlso .Item("idHotel") = 0 Then
                    Me.lblSource.Text = "Bed´s Online"
                End If



                Me.lblConvenio.Visible = (.Item("NoConvenio").ToString.Trim().Length > 0)
                Me.lblNoConvenio.Visible = (.Item("NoConvenio").ToString.Trim().Length > 0)
                Me.lblConvenio2.Visible = (.Item("NoConvenio").ToString.Trim().Length > 0)
                Me.lblEmpresaConvenio.Visible = (.Item("NoConvenio").ToString.Trim().Length > 0)
                ...
                If (CType(Me.Page, PaginaBase).IsSupervisor Or CType(Me.Page, PaginaBase).isUserChain) AndAlso (Not .IsNull("EstatusPMS")) Then
        ...
        If Not dsReservaciones Is Nothing AndAlso dsReservaciones.Tables(dsReservaciones.RESERVA_TABLE).Rows.Count > 0 Then
            With dsReservaciones.Tables(dsReservaciones.RESERVA_TABLE).Rows(0)
                ...
                If .Item(dsReservaciones.FIELD_NoReservacionGalileo).ToString <> "" Then
                    ...
                    End Try
                End If
                'si es reservacion que no está con galileo hay que validar que cumpla con las reglas del minimo dia para cancelar
                If .Item(dsReservaciones.FIELD_NoReservacionGalileo).ToString = "" Then
                    ...
                    End With
                    If idhotel <> 0 Then
                        If Not CType(Me.Page, PaginaBase).IsSupervisor AndAlso Not CType(Me.Page, PaginaBase).isUserChain AndAlso source <> "HTL" Then
                ...
                If LocalCancel(.Item(dsReservaciones.FIELD_IDRESERVACION), GalileoConfCancelNumber, CInt(Val(.Item("idUsuario").ToString)), NoCancelacion, .Item(dsReservaciones.FIELD_NORESERVACION)) Then
                    ...
                    CType(Me.Page, PaginaBase).guardalog("/HotelAdministrator/Pages/ReservationDetails.aspx", PaginaBase.acciones.Eliminar, "Canceló la reservacion " & .Item(dsReservaciones.FIELD_NORESERVACION).ToString)
                    Return True
                Else
                    Threading.Thread.CurrentThread.CurrentUICulture = gUI
                    PortalCulture.SetCulture(gUI.ToString)
    ...
    '                        enviarcorreo(dsReservaciones)
    '                        Me.guardalog("/HotelAdministrator/Pages/ReservationDetails.aspx", PaginaBase.acciones.Eliminar, "Canceló la reservacion " & .Item(dsReservaciones.FIELD_NORESERVACION).ToString)
    '                        Return True
    '                    Else
    ...
    Private Function LocalStatusPMS() As Boolean
        ...
        Try
            ...
            trans = False
            If afec > 0 Then
                CType(Me.Page, PaginaBase).guardalog("/HotelAdministrator/Pages/ReservationDetails.aspx?qs=" & IdReservacion, PaginaBase.acciones.Modificar, "Cambio status PMS, no reservacion   " & NoReserv)
                Return True
            Else
                Return False
            End If
    ...
    Private Function LocalCancel(ByVal idReservacion As Long, ByVal ConfNumCancel As String, ByVal IDUser As Integer, ByRef refNoCancelacion As String, ByVal noreservacion As String) As Boolean
        ...
        Try
            ...
            End If
            Dim msg As String
            msg = PortalCulture.GetString("00780", True) & CType(Me.Page, PaginaBase).ReadUserCookie.GetValue(0)
            Try
            ...
            sqlcmd.Parameters.Add("@iduser", SqlDbType.Int).Value = CType(Me.Page, PaginaBase).UserIdentityName

            Dim afec As Integer = sqlcmd.ExecuteNonQuery()
            transacc.Commit()
            trans = False
            If afec > 0 Then
                CType(Me.Page, PaginaBase).guardalog("/HotelAdministrator/Pages/ReservationDetails.aspx?qs=" & idReservacion, PaginaBase.acciones.Eliminar, "Cancelaron la reservacion  " & noreservacion)
                Return True
            Else
                Return False
            End If
    ...
    Private Sub enviarcorreo(ByVal dsreservaciones As ReservaDatos, Optional ByVal isCancel As Boolean = True)
    ...
    Private Sub Fillcorreo(ByVal dsreservaciones As ReservaDatos, ByVal idioma As String, ByVal _to As String, ByVal _cc As String, ByVal _bcc As String)
        ...
        Try
            With dsreservaciones.Tables(dsreservaciones.RESERVA_TABLE).Rows(0)
                ...
                If SourceReserv = "UNI" Or SourceReserv = "IDS" Or SourceReserv = "ADS" Or SourceReserv = "CCT" Then
                    Mail.AddParameter("PROPERTY_URLLOGO") = Util.Utility.LoadImagen(CType(Me.Page, PaginaBase).cInfoActual.Empresa)
                    Mail.AddParameter("PROPERTY_NAME") = CType(Me.Page, PaginaBase).cInfoActual.HotelName
                End If
    ...
    Private Function CargaPoliticasUvNetRates() As String
        Try
            Dim rssFile As String = CType(Me.Page, PaginaBase).GeRequestApplicationPath(String.Concat("/Politicas/", PortalCulture.GetCulture.ToString.Substring(0, 2), "/UvNetRatesPolicies.txt"))
        ...
        If Me.CanShowConfirmButton Then
            Try
                With New SqlCommand("spPagosReservacionesInsert", New SqlConnection(ConfigurationManager.AppSettings("HotelConnection")))
                    ...
                    .Parameters.Add("@PaymentSource", SqlDbType.Int).Value = Me.lstPayMode.SelectedItem.Value
                    Try
                        .Connection.Open()
                        .ExecuteNonQuery()
                        hr = True
