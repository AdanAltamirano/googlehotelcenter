...
#End Region
    ...
    Private Sub loadfilter()
        ...
        'item = New ListItem
        'item.Text = "Booking.com"
        'item.Value = "Booking.com"
        'ddlOTAs.Items.Add(item)

        item = New ListItem
        item.Text = "Bookit.com"
        item.Value = "Bookit.com"
        ddlOTAs.Items.Add(item)
        ...
        item = New ListItem
        item.Text = "Hotel Beds"
        item.Value = "Hotel Beds"
        ddlOTAs.Items.Add(item)
        ...
        item = New ListItem
        item.Text = "PriceTravel"
        item.Value = "PriceTravel"
        ddlOTAs.Items.Add(item)
    End Sub
    ...
    Public Function searchReservations(Optional ByVal newStatus As Integer = -1) As DataView
        ...
        Dim sAgent As String = If(ddlAgents.Visible, ddlAgents.SelectedValue, "-2")
        Dim sAgencia As String

        If idSegmento = 274 Then
            sAgencia = If(ddlAgencies.SelectedValue > -2, ddlAgencias.SelectedValue, "")
        Else
            sAgencia = If(ddlAgencias.SelectedIndex > 0, ddlAgencias.SelectedValue, "")
        End If

        If UserChain Then
            If chkHotels.Checked Then
                Return getReservations(Me.ddlHoteles.SelectedValue, CheckIn, CheckOut, RoomId, Status, NameClient, Me.ddlFilterby.SelectedItem.Value, Me.idUsuario, Agencia:=sAgencia)
            Else
                Return getReservations(Me.idHotel, CheckIn, CheckOut, RoomId, Status, NameClient, Me.ddlFilterby.SelectedItem.Value, Agencia:=sAgencia, UsuarioAgente:=sAgent)
            End If
        Else
            Return getReservations(Me.idHotel, CheckIn, CheckOut, RoomId, Status, NameClient, Me.ddlFilterby.SelectedItem.Value, Agencia:=sAgencia, UsuarioAgente:=sAgent)
        End If
    ...
    ' Store procedures
    Public Const SPGETRESERVATIONS As String = "spReservations_GetByHotel"
    ...
    Function CargaAgencias(ByVal data As DataSet) As Boolean
        ...
        If Not data Is Nothing AndAlso data.Tables.Count = 2 And data.Tables(1).Rows.Count > 0 Then
    ...
    Private Function Columnas(ByVal data As DataSet)
        ...
        CExport.AddParameter("Fee", "Comision")
    ...
    Public Function getReservations(ByVal hotelId As Integer, ByVal CheckIn As DateTime, ByVal CheckOut As DateTime, ByVal RoomId As Integer, ByVal Status As Byte, ByVal NameClient As String, ByVal filter As String, Optional ByVal idUsuario As Integer = -1, Optional ByVal Agencia As String = "", Optional ByVal UsuarioAgente As String = "-2") As DataView
            ...
            Try
                With .SelectCommand
                    ...
                    If Not String.IsNullOrEmpty(Agencia) Then
                        ...
                        .Parameters.Add(New SqlParameter("@UsuarioAgente", SqlDbType.Int)).Value = CInt(UsuarioAgente)
                    End If

                    If Me.Supervisor Then
                        .Parameters.Add(New SqlParameter("@IsSupervisor", SqlDbType.Bit)).Value = True
                    End If
        ...
        If Me.ddlCanal.SelectedValue <> "ALL" Then
            ...
            If ddlCanal.SelectedValue = "IDS" AndAlso ddlOTAs.SelectedValue <> "All" Then
                filtroStr = "SourceIds like '%" & ddlOTAs.SelectedValue & "%'" & IIf(filtroStr <> String.Empty, " and " & filtroStr, "")
            End If
        End If
    ...
    Sub callFilter()
        ...
        End If
        'sFiltro &= If(ddlOTAs.SelectedIndex = 0, "", ", OTA " & ddlOTAs.SelectedItem.Text)
